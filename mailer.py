...
def build_email_html(articles_by_category, edition="Morning"):
    ...
    html += "<div style='font-size:11px;color:#6b5d50;text-transform:uppercase;margin-top:6px;'>" + edition + " Edition - " + now + "</div>"
    html += "</div>"
    ...
    for category, articles in articles_by_category.items():
        ...
        html += "<div style='font-size:11px;font-weight:700;color:#c1440e;border-bottom:2px solid #c1440e;padding-bottom:8px;margin-bottom:20px;'>" + label + "</div>"
        for a in articles[:4]:
            title = a.get("title", "")
            source = a.get("source", "")
            url = a.get("url", "")
